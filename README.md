...
```text
Install the AIOps Vault Template from https://github.com/LinLin00000000/aiops-vault-template.

Do it autonomously:
1. Clone the repository to a temporary or project directory.
2. Set `VAULT_PATH` to `$AIOPS_ROOT` if it exists, otherwise to `~/ai-ops`.
...
```bash
git clone https://github.com/LinLin00000000/aiops-vault-template.git
cd aiops-vault-template
python3 scripts/install.py --vault ~/ai-ops --agent auto
